...
def rotate90(xn,m):
    ...
    r=scipy.misc.imresize(r,24.0/(r.shape[0]))
    g=scipy.misc.imresize(g,24.0/(g.shape[0]))
    b=scipy.misc.imresize(b,24.0/(b.shape[0]))
    xn[:,:,0]=r
    xn[:,:,1]=g
    xn[:,:,2]=b
    return xn
def scale(xn):
    ...
    xn=scipy.misc.imresize(xn2,24.0/(xn2.shape[0]))
    return xn
def crop(xn,start):
    xcrop=xn[start[0]:start[0]+24,start[1]:start[1]+24,:]
    return xcrop
def cropimg(x):
    start=numpy.random.randint(4, size=2)
    xnew=numpy.zeros((x.shape[0],24*24*1))
    temp2=numpy.zeros((x.shape[0],24,24,1))
    for i in range(x.shape[0]):         
        temp=numpy.zeros((28,28,1))
        temp = numpy.reshape(x[i],(28,28,1))   
        xnew[i]=crop(temp,start).flatten()
    return xnew
def translate_image(xvals,typ):
    for x in range(xvals.shape[0]):    
        old=xvals[x].shape
        temp=numpy.zeros((24,24,1))
        temp = numpy.reshape(xvals[x],(24,24,1))
        if typ==1:
...
# TODO
def test_dropout(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
    ...
    # Load the dataset
    # Load the dataset
    f = gzip.open('/home/ubuntu/extern/Final/dropconnect/data/mnist.pkl.gz', 'rb')
...
def test_dropout_crop(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh,p=0.5):
    """
    Wrapper function for testing LeNet on SVHN dataset
...
def test_dropout_cropscalrot(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh,p=0.5):
    """
    Wrapper function for testing LeNet on SVHN dataset
...
def test_dropconnect(learning_rate=0.1, n_epochs=1000, nkerns=[64, 128],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh,p=0.5):
    """
    Wrapper function for testing LeNet on SVHN dataset
...
def test_dropconnect_crop(learning_rate=0.1, n_epochs=1000, nkerns=[64, 128],
...
def test_dropconnect_cropscalrot(learning_rate=0.1, n_epochs=1000, nkerns=[64, 128],
            batch_size=120, verbose=False, fileName = 'predictions',activation=tanh,fullyconnected=20,p=0.5):
    """
    Wrapper function for testing LeNet on SVHN dataset
...
def test_lenet(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh):
    """
    Wrapper function for testing LeNet on SVHN dataset
    ...
    # Load the dataset
    # Load the dataset
    f = gzip.open('/home/ubuntu/dropconnect/data/mnist.pkl.gz', 'rb')
    train_set, valid_set, test_set = cPickle.load(f)
    f.close()
...
def test_lenet_crop(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh):
    """
    Wrapper function for testing LeNet on SVHN dataset
...
def test_lenet_cropscalrot(learning_rate=0.01, n_epochs=1000, nkerns=[32, 64],
            batch_size=120, verbose=False, fileName = 'predictions',fullyconnected=20,activation=tanh):
    """
    Wrapper function for testing LeNet on SVHN dataset
